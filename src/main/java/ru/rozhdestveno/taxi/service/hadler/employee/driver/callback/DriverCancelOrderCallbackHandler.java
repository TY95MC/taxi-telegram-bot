...
@Component
@Slf4j
public class DriverCancelOrderCallbackHandler extends EmployeeHandler {
    private final OrderRepository orderRepository;
    private final CustomerRepository customerRepository;
    private final EmployeeRepository employeeRepository;

    public DriverCancelOrderCallbackHandler(OrderRepository orderRepository, CustomerRepository customerRepository,
    ...
    public List<PartialBotApiMethod<?>> handleRequest(Employee employee, String message) {
        boolean condition = employee.getStatus().equals(EmployeeStatus.DRIVER) && message.startsWith("/cancel_order ");
        ...
        message = message.replace("/cancel_order ", "");
        ...
        if (order.getClient() != null) {
            messages.add(BotUtil.createMessage(order.getClient().getId(), "Заказ отменен"));
            order.getClient().setState(START);
            customerRepository.saveAndFlush(order.getClient());
        }

        orderRepository.saveAndFlush(order);
        messages.add(BotUtil.createMessage(employee.getId(), "Заказ <code>" + order.getId() + "</code> отменен!"));
