...
@Component
@Slf4j
public class DriverArrivedCallbackHandler extends EmployeeHandler {
    private final OrderRepository orderRepository;

    public DriverArrivedCallbackHandler(OrderRepository orderRepository) {
        this.orderRepository = orderRepository;
    }

    @Override
    public List<PartialBotApiMethod<?>> handleRequest(Employee employee, String message) {
        boolean condition = employee.getStatus().equals(EmployeeStatus.DRIVER) && message.startsWith("/arrived ");

        if (!condition && next != null) {
            return next.handleRequest(employee, message);
        }

        long orderId = Long.parseLong(message.replace("/arrived ", ""));
        ...
        return List.of(BotUtil.createMessage(order.getClient().getId(),
